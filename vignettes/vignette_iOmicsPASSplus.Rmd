...
### in R console
After cloning the repository **iOmicsPASSplus** from github or unzipping the zip folder, in R, set the working directory to the extracted folder and install the R-package as follows:
```{r eval=FALSE}
...
To illustrate the use of the various modules in the R-package, we will utilize the plasma protein and microRNA datasets from the [Khoo et al., 2019](https://www.frontiersin.org/articles/10.3389/fphys.2019.00379/full) measured among 8 obese insulin-resistant (OIR, HOMA-IR>2.5) and 9 lean insulin-sensitive (LIS, HOMA-IR<1.0) normoglyceric males. The dataset `PhenotypeFile` describes the phenotype group of the 17 study participants as well as their age and BMI.
...
In the supervised approach, the sample covariance matrix $S$ is computed and corrected to the nearest positive semi-definite (PSD) matrix if any of its eigenvalues are negative using the approach by [Nicholas J. Higham, 1988](https://www.sciencedirect.com/science/article/pii/0024379588902236). Upon ensuring that the covariance matrix satisfies the PSD property, graphical LASSO in `huge` R package is carried out next. A vector of lambda values are used to tune the $L_1$-penalty term in the model to yield a grid of corresponding penalized log-likelihoods. 
...
$$\lambda_{grid} =\{\lambda_1,\lambda_2,...,\lambda_{30}\}= exp\{log(1),log(0.853),…,log(0.01)\}.$$
  ...
  The selected regularization parameter $\lambda$ is then used to refit the GLASSO model to yield the corresponding precision matrix and the partial correlation matrix. The non-zero entries in this matrix is then converted into an edge-level network file, indicating the direction of association, to be used for running the predictive analysis module `iOmicsPASS.R()`. The partial correlation between molecule $𝑖$ and $𝑗$ is calculated by [Mohsen Pourahmadi, 2011](https://projecteuclid.org/journals/statistical-science/volume-26/issue-3/Covariance-Estimation-The-GLM-and-Regularization-Perspectives/10.1214/11-STS358.full):
...
\text{Matrix} S^* =(s^*_{ij})=\begin{cases}
  ...
  &\frac{|b^\prime_{ij}|}{b_{max}}, &&\textit{otherwise}.
\end{cases}
\end{aligned}$$
...
Similarly, there will be four `.txt` file output. Instead of `Estimated_Network_glasso.txt`, `Fusednetwork_hybridmethod.txt` will be created:
...
* `PartialCorrelation_icov.txt`\
  A sparse $p\times p$ data matrix describing the partial correlation between each pair of feature converted from the estimated inverse covariance matrix or the precision matrix.
*Fusednetwork_hybridmethod.txt

* `Fusednetwork_hybridmethod.txt`\
A data file with seven columns (i.e. nodeA, nodeB, dir, partialcor, DatatypeA, DatatypeB, EdgeType) and each row describing a pair of feature that have an non-zero entry in the estimated precision matrix. The data is in the format required as a network file in running the predictive analysis module `iOmicsPASS.R`.
  
  **Note:** Both `Combined_data.txt` and `Fusednetwork_hybridmethod.txt` will be copied and placed in `/iOmicsPASS/inputFiles/` folder automatically for the predictive subnetwork discovery module.

## Subnetwork discovery module #iOmicsPASS.R()

This module carries out the predictive analysis of subnetwork discovery. The function integrates multiple -omics datasets utilizing biological network information and uses a shrunken centroid algorithm modified from PAM by [Tibshirani et al., 2002](https://www.pnas.org/content/99/10/6567) to pick **network signatures** that can best distinguish between different phenotypes. It is a supervised method where the classification of the samples is known in the training data. The tool can be applied in clinical settings, to better understand the biological mechanisms underlying the differences across groups of samples (e.g. when comparing disease versus non-disease group) in studies with multiple related -omics data.

The details of the method can be found in the [Koh et al., 2019](https://www.nature.com/articles/s41540-019-0099-y). In this version released, we have made the tool more flexible and allow for predictive analysis on a single dataset with a single network to construct interaction edges within the features in the data. It also allow for specification of signed interactions in the network file to cater to positive regulation or negative inhibition of expression levels. 


### Input Parameter file for iOmicsPASS

Before running `iOmicsPASS.R()`, we need to create a parameter file using `createInputParam()` function to feed the tool the necessary information to carry out the analysis.
...
|**`normalizedBy`**| *boolean* <TRUE/FALSE> |Either "Y" or "Z". if "Y", interaction scores in data.X are normalized by same feature in data.Y and if "Z", scores in X are normalized by same feature in data.Z.|
...
Alternatively, we can use the estimated correlation network earlier (e.g. from hybrid approach) to run iOmicsPASS by creating a input parameter file specifying the file name of the concatenated data `Combined_data.txt` and estimated network `Fusednetwork_hybridmethod.txt`. Since the data has already been standardized and log-transformed, we will set `log.transform=FALSE` and `standardize.data = FALSE`.
```{r, eval=F}
...
Using a phenotype file, we set the row names as the sample identifier and the file should include the grouping variable and other clinical variables that you want to adjust for.
...
Let us create prior probabilities using age and BMI and tell `createPrior` function that the grouping variables is labelled "Group". In this example, there are no categorical variables but if there are any, you will have to specify the column labels using `var.cat`. Else, the function will assume all are continuous variables.
...
Next, we re-run iOmicsPASS+ with the new prior probabilities and set `usePrior = TRUE`.
```{r, eval=F}
...
**Note**: The examples are not completely reproducible due to the random number generator used for cross-validation. Thus, there may be small discrepancies. We also note that every data set is different and no single approach is typically recommended. Instead, one should explore different analysis approach to pick the best one that fit his or her data. 

**_As Occam's razor suggests, simplicity is the best_**
